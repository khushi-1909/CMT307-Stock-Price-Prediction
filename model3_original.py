#=============================================================================
# Imports and quiet-mode environment flags
#=============================================================================

#Stop TensorFlow from printing log messages.
import os, sys
...
def get_market_data(period):
    d = yf.download("^VIX", period=period, auto_adjust=True, progress=False)
    if d.empty:
        return pd.DataFrame()
...
#Label is 1 if volatility goes up over the next 10 days, 0 if it goes down.
def vol_direction_labels(close, lookahead, past_window=20):
    rets = close.pct_change()
    past_vol = rets.rolling(past_window).std()
    fwd_vol  = rets.rolling(lookahead).std().shift(-lookahead)
    return np.where(fwd_vol.isna() | past_vol.isna(), np.nan, (fwd_vol > past_vol).astype(float))
...
def run_fold(payload):
    ...
    val_accs = [(t, ((val_ens > t).astype(int) == y_val).mean()) for t in thr_grid]
    best_thr = max(val_accs, key=lambda x: x[1])[0]
...
def plot_results(results, y, yhat, probs):
    ...
    fig.suptitle(f"{TICKER} Volatility-Direction Prediction ({PERIOD})",
    ...
    ax.set_xticklabels(["Pred VolDown", "Pred VolUp"])
    ax.set_yticklabels(["True VolDown", "True VolUp"])
...
#Download MSFT data, build all features, add VIX columns and the up/down label.
def load_dataset():
    ...
    df = add_features(raw.copy()).join(get_market_data(PERIOD), how="left")
    ...
    df["Direction"] = vol_direction_labels(df["Close"], LOOKAHEAD, past_window=20)
    return df.dropna()
...
if __name__ == "__main__":
    ...
    #Run the trading simulation on the fold predictions.
    test_idx = np.concatenate([np.arange(a[4], a[5]) for a in fold_args])
    preds_df = pd.DataFrame({"Predictions": all_pred}, index=df.index[test_idx])
    run_full_simulation(df, preds_df, TICKER, PERIOD, horizon=90)
